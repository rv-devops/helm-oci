...
: ${HELM_PLUGIN_DIR:="$(helm home --debug=false)/plugins/helm-oci"}
...
getDownloadURL() {
  ...
  if [ -n "$version" ]; then
    DOWNLOAD_URL="https://github.com/$PROJECT_GH/releases/download/$version/helm-oci-$OS.tgz"
  else
...
fail_trap() {
  ...
  if [ "$result" != "0" ]; then
    ...
    printf '\tFor support, go to https://github.com/johnlinvc/helm-oci.\n'
  fi
  exit $result
}
